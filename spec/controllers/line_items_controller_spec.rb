RSpec.describe LineItemsController, type: :controller do
  ...
  describe 'with DESTROY #delete' do
    ...
    context 'when user_one  is signed' do
      ...
      it 'user cannot destroy line_items of another`s user' do
      ...
      it 'user cannot destroy guest`s line_item' do
        expect(LineItem.count).to eq(3)
      ...
      it 'user  can destroy only own line_items' do
        expect(LineItem.count).to eq(3)
  ...
  describe 'with POST #create' do
    ...
    context 'when user is signed ' do
      ...
      it 'user create line_item with success in own order' do
        expect { post(:create, params: valid_params) }.to change(LineItem, :count).by(1)
      end

      it 'user can not create line_item with wrong book_id' do
  ...
  describe 'with PUT #update' do
    ...
    context 'when user_one is signed' do
      ...
      it 'user try change quantity with empty quantity_action' do
      ...
      it 'user increment quantity of own line_item' do
      ...
      it 'can not increment quantity of another`s line_item' do
      ...
      it 'decrement quantity of own line_item' do
      ...
      it 'can not decrement quantity of another`s line_item' do
        put :update, params: { order_id: order_two.id, id: line_item_two.id, quantity_action: 'increment' }
        expect(order_two.line_items.find_by(id: line_item_two.id).quantity).to eq(current_quantity)
      end
    end

    context 'when guest have order' do
      before do
        session['order_id'] = order_guest.id
      end

      it 'can increment quantity of own line_item' do
      ...
      it 'can decrement quantity of own line_item' do
