RSpec.describe 'CheckoutAddress', type: :feature do
  ...
  describe 'when order has not line_items' do
    before do
      sign_in user
      create(:book)
      visit root_path
    ...
    it 'redirect to cart page, because not exist items' do
      expect(address_page.flash_fail_message).to have_content(I18n.t('checkout.alert.cart_empty'))
      expect(address_page).to have_current_path order_line_items_path(Order.first)
    end
  end

  describe 'when order has line_items' do
    before do
      sign_in user
      create(:book)
      visit root_path
